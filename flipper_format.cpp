...
bool flipper_format_file_open_existing(FlipperFormat* flipper_format, const char* path) {
    ...
    return file_stream_open(flipper_format->stream, path, "rw");
}
...
bool flipper_format_file_open_append(FlipperFormat* flipper_format, const char* path) {
    ...
    bool result =
        file_stream_open(flipper_format->stream, path, "rw");
...
bool flipper_format_read_header(
    ...
    uint32_t* version) {
    // furi_assert(flipper_format);
    // Serial.println("flipper_format_read_string");
    // Serial.println(flipper_format_read_string(flipper_format, flipper_format_filetype_key, filetype));
