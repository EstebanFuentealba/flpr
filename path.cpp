...
void path_extract_filename_no_ext(const char* path, FuriString* filename) {
    furi_string_set_str(filename, path);

    size_t start_position = furi_string_search_rchar(filename, furi_string_alloc_set_str('/'), 3);
    size_t end_position = furi_string_search_rchar(filename, furi_string_alloc_set_str('.'), 3);
...
void path_extract_filename(FuriString* path, FuriString* name, bool trim_ext) {
    size_t filename_start = furi_string_search_rchar(path, furi_string_alloc_set_str('/'), 3);
    if(filename_start > 0) {
        filename_start++;
        furi_string_set_n(name, path, filename_start, furi_string_size(path) - filename_start);
    }
    if(trim_ext) {
        size_t dot = furi_string_search_rchar(name, furi_string_alloc_set_str('.'), 3);
        if(dot > 0) {
...
void path_extract_extension(FuriString* path, char* ext, size_t ext_len_max) {
    size_t dot = furi_string_search_rchar(path, furi_string_alloc_set_str('.'), 3);
    size_t filename_start = furi_string_search_rchar(path, furi_string_alloc_set_str('/'), 3);
...
static inline void path_cleanup(FuriString* path) {
    furi_string_trim(furi_string_alloc_set_str("  \n\r\t"), furi_string_get_cstr(path));
    while(furi_string_end_with(path,furi_string_alloc_set_str("/"))) {
        furi_string_left(path, furi_string_size(path) - 1);
    }
}

void path_extract_basename(const char* path, FuriString* basename) {
    furi_string_set_str(basename, path);
    path_cleanup(basename);
    size_t pos = furi_string_search_rchar(basename, furi_string_alloc_set_str('/'), 3);
...
void path_extract_dirname(const char* path, FuriString* dirname) {
    furi_string_set_str(dirname, path);
    path_cleanup(dirname);
    size_t pos = furi_string_search_rchar(dirname, furi_string_alloc_set_str('/'), 3);
    if(pos != FURI_STRING_FAILURE) {
...
void path_append(FuriString* path, const char* suffix) {
    ...
    suffix_str = furi_string_alloc_set_str(suffix);
...
void path_concat(const char* path, const char* suffix, FuriString* out_path) {
    furi_string_set_str(out_path, path);
