...
IconAnimation* icon_animation_alloc(const Icon* icon) {
    // furi_assert(icon);
    IconAnimation* instance = (IconAnimation*)malloc(sizeof(IconAnimation));
    instance->icon = icon;
    instance->timer =
...
void icon_animation_timer_callback(void* context) {
    ...
    IconAnimation* instance = (IconAnimation*)context;
...
void icon_animation_start(IconAnimation* instance) {
    ...
    if(!instance->animating) {
        ...
        xTimerChangePeriod(
                instance->timer,
                (furi_kernel_get_tick_frequency() / instance->icon->frame_rate), portMAX_DELAY);
        // furi_check(
        //     xTimerChangePeriod(
        //         instance->timer,
