...
void desktop_lock_menu_set_dummy_mode_state(DesktopLockMenuView* lock_menu, bool dummy_mode) {
    DesktopLockMenuViewModel* model = (DesktopLockMenuViewModel*)view_get_model(lock_menu->view);
    model->dummy_mode = dummy_mode;
    view_commit_model(lock_menu->view, true);
...
void desktop_lock_menu_draw_callback(Canvas* canvas, void* model) {
    DesktopLockMenuViewModel* m = (DesktopLockMenuViewModel*)model;
...
bool desktop_lock_menu_input_callback(InputEvent* event, void* context) {
    ...
    DesktopLockMenuView* lock_menu = ( DesktopLockMenuView*)context;
    uint8_t idx = 0;
    bool consumed = false;
    ...
    view_commit_model(lock_menu->view, update);
...
DesktopLockMenuView* desktop_lock_menu_alloc() {
    DesktopLockMenuView* lock_menu = (DesktopLockMenuView*)malloc(sizeof(DesktopLockMenuView));
