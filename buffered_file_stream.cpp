...
typedef struct {
    FZStream stream_base;
    FZStream* file_stream;
    StreamCache* cache;
    bool sync_pending;
} BufferedFileStream;
...
bool buffered_file_stream_open(
    ...
    return file_stream_open(stream->file_stream, path, open_mode);
}

bool buffered_file_stream_close(FZStream* _stream) {
    // furi_assert(_stream);
    BufferedFileStream* stream = (BufferedFileStream*)_stream;
    // furi_check(stream->stream_base.vtable == &buffered_file_stream_vtable);
    bool success = false;
...
bool buffered_file_stream_sync(FZStream* _stream) {
    // furi_assert(_stream);
    BufferedFileStream* stream = (BufferedFileStream*)_stream;
    // furi_check(stream->stream_base.vtable == &buffered_file_stream_vtable);
...
FS_Error buffered_file_stream_get_error(FZStream* _stream) {
    // furi_assert(_stream);
    BufferedFileStream* stream = (BufferedFileStream*)_stream;
    // furi_check(stream->stream_base.vtable == &buffered_file_stream_vtable);
...
static void buffered_file_stream_free(BufferedFileStream* stream) {
    // furi_assert(stream);
    buffered_file_stream_sync((FZStream*)stream);
...
static bool buffered_file_stream_seek(
    ...
    if((new_offset != 0) || (offset_type != StreamOffsetFromCurrent)) {
        if(stream->sync_pending) {
            success = buffered_file_stream_sync((FZStream*)stream);
        } else {
            stream_cache_drop(stream->cache);
        }
        if(success) {
