...
bool tar_archive_open(TarArchive* archive, const char* path, TarOpenMode mode) {
    ...
    FZFile* stream = storage_file_alloc(archive->storage);
    if(!storage_file_open(stream, path, open_mode)) {
        storage_file_free(stream);
        return false;
    }
...
static bool archive_extract_current_file(TarArchive* archive, const char* dst_path) {
    ...
    FZFile* out_file = storage_file_alloc(archive->storage);
    ...
    do {
        while(n_tries-- > 0) {
            if(storage_file_open(out_file, dst_path, FILE_WRITE)) {
                break;
            }
...
static int archive_extract_foreach_cb(mtar_t* tar, const mtar_header_t* header, void* param) {
    ...
    FuriString* converted_fname = furi_string_alloc_set_str(header->name);
...
bool tar_archive_add_file(
    ...
    bool success = false;
    FZFile* src_file = storage_file_alloc(archive->storage);
    uint8_t n_tries = FILE_OPEN_NTRIES;
    do {
        while(n_tries-- > 0) {
            if(storage_file_open(src_file, fs_file_path, FILE_WRITE)) {
                break;
            }
...
bool tar_archive_add_dir(TarArchive* archive, const char* fs_full_path, const char* path_prefix) {
    ...
    FZFile* directory = storage_file_alloc(archive->storage);
    FileInfo file_info;
    ...
    do {
        ...
        while(true) {
            ...
            if(strlen(path_prefix)) {
                ...
                furi_string_set_str(element_name, name);
            }
