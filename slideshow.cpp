...
Slideshow* slideshow_alloc() {
    Slideshow* ret = (Slideshow*)malloc(sizeof(Slideshow));
    ret->loaded = false;
    return ret;
}
...
bool slideshow_load(Slideshow* slideshow, const char* fspath) {
    ...
    FZFile* slideshow_file = (FZFile*)storage_file_alloc(storage);
    slideshow->loaded = false;
    do {
        if(!storage_file_open(slideshow_file, fspath, FILE_READ)) {
            break;
        }
        ...
        Icon* icon = &slideshow->icon;
        FURI_CONST_ASSIGN_(short, icon->frame_count, header.frame_count);
        // FURI_CONST_ASSIGN(icon->frame_count, header.frame_count);
        FURI_CONST_ASSIGN_(short, icon->width, header.width);
        // FURI_CONST_ASSIGN(icon->width, header.width);
        FURI_CONST_ASSIGN_(short, icon->height, header.height);
        // FURI_CONST_ASSIGN(icon->height, header.height);
        icon->frames = (uint8_t *const *)malloc(header.frame_count * sizeof(uint8_t*));
