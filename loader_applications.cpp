...
LoaderApplications* loader_applications_alloc(void (*closed_cb)(void*), void* context) {
    LoaderApplications* loader_applications = (LoaderApplications*)malloc(sizeof(LoaderApplications));
...
static LoaderApplicationsApp* loader_applications_app_alloc() {
    LoaderApplicationsApp* app = (LoaderApplicationsApp*)malloc(sizeof(LoaderApplicationsApp)); //-V799
    app->fap_path = (FuriString*)furi_string_alloc_set_str(EXT_PATH("apps"));
    app->dialogs = (DialogsApp*)furi_record_open(RECORD_DIALOGS);
    app->storage = (Storage*)furi_record_open(RECORD_STORAGE);
    app->loader = (Loader *)furi_record_open(RECORD_LOADER);
...
static bool loader_applications_item_callback(
    ...
    LoaderApplicationsApp* loader_applications_app = (LoaderApplicationsApp*)context;
...
static void loader_applications_start_app(LoaderApplicationsApp* app) {
    ...
    FuriPubSubSubscription* subscription =
        furi_pubsub_subscribe(loader_get_pubsub(app->loader), loader_pubsub_callback, NULL);
...
static int32_t loader_applications_thread(void* p) {
    LoaderApplications* loader_applications = (LoaderApplications*) p;
