...
static void bubble_animation_draw_callback(Canvas* canvas, void* model_) {
    ...
    BubbleAnimationViewModel* model = (BubbleAnimationViewModel*)model_;
    const BubbleAnimation* animation = model->current;
    ...
    canvas_draw_bitmap( canvas, 0, y_offset, width, height, animation->icon_animation.frames[index]);
...
static const FrameBubble*
    ...
    if(active && model->active_bubbles == 0) {
        return NULL;
    } else if(!active && model->passive_bubbles == 0) {
        return NULL;
    }
...
static bool bubble_animation_input_callback(InputEvent* event, void* context) {
    ...
    BubbleAnimationView* animation_view = (BubbleAnimationView*)context;
    bool consumed = false;
...
static void bubble_animation_timer_callback(void* context) {
    // furi_assert(context);
    BubbleAnimationView* view = (BubbleAnimationView*)context;
    bool activate = false;
...
static Icon* bubble_animation_clone_first_frame(const Icon* icon_orig) {
    ...
    Icon* icon_clone = (Icon*)malloc(sizeof(Icon));
    memcpy(icon_clone, icon_orig, sizeof(Icon));

    icon_clone->frames = (uint8_t *const *)malloc(sizeof(uint8_t*));
    ...
    FURI_CONST_ASSIGN_(short, icon_clone->frame_count, 1);
    return icon_clone;
}
...
BubbleAnimationView* bubble_animation_view_alloc(void) {
    BubbleAnimationView* view = (BubbleAnimationView*)malloc(sizeof(BubbleAnimationView));
