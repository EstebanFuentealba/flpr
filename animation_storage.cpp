...
static bool animation_storage_load_single_manifest_info(
    ...
    do {
        ...
        manifest_info->name = (char*)malloc(furi_string_size(read_string) + 1);
...
void animation_storage_fill_animation_list(StorageAnimationList_t* animation_list) {
    ...
    do {
        ...
        if(furi_string_cmp_str(read_string, "Flipper Animation Manifest")) break;
        do {
            storage_animation = (StorageAnimation*)malloc(sizeof(StorageAnimation));
            ...
            storage_animation->manifest_info.name = (char*)malloc(furi_string_size(read_string) + 1);
            strcpy(
...
static bool animation_storage_cast_align(FuriString* align_str, Align* align) {
    if(!furi_string_cmp_str(align_str, "Bottom")) {
        *align = AlignBottom;
    } else if(!furi_string_cmp_str(align_str, "Top")) {
        *align = AlignTop;
    } else if(!furi_string_cmp_str(align_str, "Left")) {
        *align = AlignLeft;
    } else if(!furi_string_cmp_str(align_str, "Right")) {
        *align = AlignRight;
    } else if(!furi_string_cmp_str(align_str, "Center")) {
        *align = AlignCenter;
    } else {
        return false;
    }
...
static bool animation_storage_load_frames(
    ...
    FURI_CONST_ASSIGN_(short, icon->frame_count, max_frame_count + 1);
    FURI_CONST_ASSIGN_(short, icon->frame_rate, 0);
    FURI_CONST_ASSIGN_(short, icon->height, height);
    FURI_CONST_ASSIGN_(short, icon->width, width);
    icon->frames = (const uint8_t* const *)malloc(sizeof(const uint8_t*) * icon->frame_count);

    bool frames_ok = false;
    FZFile* file = (FZFile*)storage_file_alloc(storage);
    FileInfo file_info;
    FuriString* filename;
    ...
    for(int i = 0; i < icon->frame_count; ++i) {
        ...
        if(!storage_file_open(
               file, furi_string_get_cstr(filename), "rw")) {
...
static bool animation_storage_load_bubbles(BubbleAnimation* animation, FlipperFormat* ff) {
    ...
    do {
        ...
        animation->frame_bubble_sequences =
            (FrameBubble *const *)malloc(sizeof(FrameBubble*) * animation->frame_bubble_sequences_count);
        ...
        for(;;) {
            ...
            FURI_CONST_ASSIGN_(short, bubble->bubble.x, u32value);
            ...
            FURI_CONST_ASSIGN_(short, bubble->bubble.y, u32value);

            if(!flipper_format_read_string(ff, "Text", str)) break;
            if(furi_string_size(str) > 100) break;

            furi_string_replace_all_str(str, "\\n", "\n");
            ...
            FURI_CONST_ASSIGN_(short, bubble->start_frame, u32value);
            if(!flipper_format_read_uint32(ff, "EndFrame", &u32value, 1)) break;
            // FURI_CONST_ASSIGN(bubble->end_frame, u32value);
             FURI_CONST_ASSIGN_(short, bubble->end_frame, u32value);
        }
...
static BubbleAnimation* animation_storage_load_animation(const char* name) {
    // furi_assert(name);
    BubbleAnimation* animation = (BubbleAnimation*)malloc(sizeof(BubbleAnimation));
    ...
    do {
        ...
        u32array = (uint32_t*)malloc(sizeof(uint32_t) * frames);
        if(!flipper_format_read_uint32(ff, "Frames order", u32array, frames)) break;
        animation->frame_order = (uint8_t*)malloc(sizeof(uint8_t) * frames);
        for(int i = 0; i < frames; ++i) {
            // FURI_CONST_ASSIGN(animation->frame_order[i], u32array[i]);
            FURI_CONST_ASSIGN_(short, animation->frame_order[i], u32array[i]);
        }
        ...
        FURI_CONST_ASSIGN_(short, animation->icon_animation.frame_rate, u32value);
