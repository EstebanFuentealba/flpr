...
void dolphin_butthurt_timer_callback(TimerHandle_t xTimer) {
    Dolphin* dolphin = (Dolphin*)pvTimerGetTimerID(xTimer);
    // furi_assert(dolphin);
...
Dolphin* dolphin_alloc() {
    Dolphin* dolphin = (Dolphin*)malloc(sizeof(Dolphin));
...
void dolphin_event_send_async(Dolphin* dolphin, DolphinEvent* event) {
    ...
    // furi_assert(event);
    event->flag = NULL;
    if(furi_message_queue_put(dolphin->event_queue, event, FuriWaitForever) == FuriStatusOk) {}
    // furi_check(furi_message_queue_put(dolphin->event_queue, event, FuriWaitForever) == FuriStatusOk);
}
...
int32_t dolphin_srv(void* p) {
    ...
    Serial.println("[dolphin] dolphin_srv");
    ...
    dolphin->state->data.butthurt = 1;
    dolphin->state->data.butthurt_daily_limit = 10;
    dolphin->state->data.flags = 0;
    dolphin->state->data.icounter = 0;
    // dolphin->state->data.icounter_daily_limit = 10;
    dolphin->state->data.timestamp = 0;
