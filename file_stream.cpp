...
bool file_stream_open(
    ...
    return storage_file_open(stream->file, path, open_mode);
}

bool file_stream_close(FZStream* _stream) {
    // furi_assert(_stream);
    FileStream* stream = (FileStream*)_stream;
    // furi_check(stream->stream_base.vtable == &file_stream_vtable);
...
FS_Error file_stream_get_error(FZStream* _stream) {
    // furi_assert(_stream);
    FileStream* stream = (FileStream*)_stream;
    // furi_check(stream->stream_base.vtable == &file_stream_vtable);
...
static bool file_stream_delete_and_insert(
    ...
    const void* ctx) {
    bool result = false;
    FZStream* stream = (FZStream*)_stream;

    // open scratchpad
    FZStream* scratch_stream = file_stream_alloc(_stream->storage);
