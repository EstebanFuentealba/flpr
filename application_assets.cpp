...
static FuriString* flipper_application_assets_alloc_app_full_path(FuriString* app_name) {
    ...
    FuriString* full_path = furi_string_alloc_set_str(APPS_ASSETS_PATH "/");
...
static FuriString* flipper_application_assets_alloc_signature_file_path(FuriString* app_name) {
    ...
    furi_string_cat_str(signature_file_path, "/" FLIPPER_APPLICATION_ASSETS_SIGNATURE_FILENAME);

    return signature_file_path;
}

static uint8_t* flipper_application_assets_alloc_and_load_data(FZFile* file, size_t* size) {
    // furi_assert(file);
...
static bool flipper_application_assets_process_files(
    ...
    FZFile* destination = (FZFile*)storage_file_alloc(storage);
    ...
    for(uint32_t i = 0; i < files_count; i++) {
        ...
        furi_string_cat_str(file_path, "/");
        furi_string_cat_str(file_path, path);

        if(!storage_file_open(
               destination, furi_string_get_cstr(file_path), FILE_WRITE)) {
...
static bool flipper_application_assets_process_dirs(
    ...
    do {
        ...
        for(uint32_t i = 0; i < dirs_count; i++) {
            ...
            furi_string_cat_str(dir_path, path);
...
static AssetsSignatureResult flipper_application_assets_process_signature(
    ...
    FZFile* signature_file = (FZFile*)storage_file_alloc(storage);
    ...
    do {
        ...
        *signature_data =
        ...
        uint8_t* signature_file_data = (uint8_t*)malloc(signature_size);
