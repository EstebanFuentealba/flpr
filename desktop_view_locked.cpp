...
static void locked_view_timer_callback(TimerHandle_t timer) {
    DesktopViewLocked* locked_view = ( DesktopViewLocked*)pvTimerGetTimerID(timer);
...
static void desktop_view_locked_update_hint_icon_timeout(DesktopViewLocked* locked_view) {
    DesktopViewLockedModel* model = (DesktopViewLockedModel*)view_get_model(locked_view->view);
...
static void desktop_view_locked_draw(Canvas* canvas, void* model) {
    DesktopViewLockedModel* m = (DesktopViewLockedModel*)model;
...
static bool desktop_view_locked_input(InputEvent* event, void* context) {
    ...
    DesktopViewLocked* locked_view = (DesktopViewLocked*)context;
    DesktopViewLockedModel* model = (DesktopViewLockedModel*)view_get_model(locked_view->view);
...
DesktopViewLocked* desktop_view_locked_alloc() {
    DesktopViewLocked* locked_view = (DesktopViewLocked*)malloc(sizeof(DesktopViewLocked));
