...
LoaderStatus loader_start_with_gui_error(Loader* loader, const char* name, const char* args) {
    ...
    if(status == LoaderStatusErrorUnknownApp &&
       loader_find_external_application_by_name(name) != NULL) {
        ...
        furi_string_replace_str(error_message, "/ext/apps/", "", 0);
        furi_string_replace_str(error_message, ", ", "\n", 0);
        furi_string_replace_str(error_message, ": ", "\n", 0);
...
static LoaderStatus loader_make_success_status(FuriString* error_message) {
    if(error_message) {
        furi_string_set_str(error_message, "App started");
    }
...
static LoaderStatus loader_start_external_app(
    ...
    do {
        loader->app.fap = flipper_application_alloc(storage);
        size_t start = furi_get_tick();

        // FURI_LOG_I(TAG, "Loading %s", path);

        FlipperApplicationPreloadStatus preload_res = flipper_application_preload(loader->app.fap, path);
        ...
        FlipperApplicationLoadStatus load_status = flipper_application_map_to_memory(loader->app.fap);
