...
static bool animation_manager_is_valid_idle_animation(
    const StorageAnimationManifestInfo* info ,const DolphinStats* stats);
...
static void animation_manager_check_blocking_callback(const void* message, void* context) {
    ...
    case StorageEventTypeCardMountError:
        // furi_assert(context);
        AnimationManager* animation_manager =(AnimationManager*) context;
...
AnimationManager* animation_manager_alloc(void) {
    ...
    Dolphin* dolphin = (Dolphin*)furi_record_open(RECORD_DOLPHIN);
    animation_manager->pubsub_subscription_dolphin = furi_pubsub_subscribe(dolphin_get_pubsub(dolphin), animation_manager_check_blocking_callback, animation_manager);
    furi_record_close(RECORD_DOLPHIN);
...
static bool animation_manager_is_valid_idle_animation(const StorageAnimationManifestInfo* info,const DolphinStats* stats) {
    // furi_assert(info);
...
void animation_manager_unload_and_stall_animation(AnimationManager* animation_manager) {
    ...
    furi_string_set_str(animation_manager->freezed_animation_name, meta->name);
