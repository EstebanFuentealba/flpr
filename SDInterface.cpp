...
void SDInterface::runUpdate() {
  ...
  display_obj.display.setTextColor(SSD1306_WHITE);

  display_obj.display.println(F(text15));
#endif
  ...
  if (updateBin) {
    if (updateBin.isDirectory()) {
      ...
      display_obj.display.println(F(text_table2[0]));
#endif
    ...
    if (updateSize > 0) {
#ifdef HAS_SCREEN
      display_obj.display.println(F(text_table2[1]));
#endif
...
#ifdef HAS_SCREEN
      display_obj.display.setTextColor(WHITE);
      display_obj.display.println(F(text_table2[2]));
#endif
...
#ifdef HAS_SCREEN
    display_obj.display.println(F(text_table2[3]));
#endif
...
#ifdef HAS_SCREEN
    display_obj.display.setTextColor(WHITE);
    display_obj.display.println(F(text_table2[4]));
#endif
...
void SDInterface::performUpdate(Stream& updateSource, size_t updateSize) {
...
#ifdef HAS_SCREEN
    display_obj.display.println(text_table2[5] + String(updateSize));
    display_obj.display.println(F(text_table2[6]));
#endif
    size_t written = Update.writeStream(updateSource);
    if (written == updateSize) {
#ifdef HAS_SCREEN
      display_obj.display.println(text_table2[7] + String(written) + text_table2[10]);
#endif
      Serial.println("Written : " + String(written) + " successfully");
    }
    else {
#ifdef HAS_SCREEN
      display_obj.display.println(text_table2[8] + String(written) + "/" + String(updateSize) + text_table2[9]);
#endif
...
#ifdef HAS_SCREEN
        display_obj.display.println(F(text_table2[11]));
#endif
...
#ifdef HAS_SCREEN
        display_obj.display.setTextColor(WHITE);
        display_obj.display.println(text_table2[12]);
#endif
...
#ifdef HAS_SCREEN
      display_obj.display.println(text_table2[13] + String(Update.getError()));
#endif
  ...
  {
#ifdef HAS_SCREEN
    display_obj.display.println(text_table2[14]);
#endif
