...
static bool flipper_format_stream_write(FZStream* stream, const void* data, size_t data_size) {
    size_t bytes_written = stream_write(stream, (const u_int8_t*) data, data_size);
    return bytes_written == data_size;
}

static bool flipper_format_stream_write_key(FZStream* stream, const char* key) {
    bool result = false;
...
bool flipper_format_stream_write_eol(FZStream* stream) {
    return flipper_format_stream_write(stream, &flipper_format_eoln, 1);
}

static bool flipper_format_stream_read_valid_key(FZStream* stream, FuriString* key) {
...
bool flipper_format_stream_seek_to_key(FZStream* stream, const char* key, bool strict_mode) {
    bool found = false;
    FuriString* read_key;
...
static bool flipper_format_stream_read_value(FZStream* stream, FuriString* value, bool* last) {
...
static bool flipper_format_stream_read_line(FZStream* stream, FuriString* str_result) {
...
static bool flipper_format_stream_seek_to_next_line(FZStream* stream) {
    const size_t buffer_size = 32;
    uint8_t buffer[buffer_size];
    bool result = false;
    bool error = false;
...
bool flipper_format_stream_write_value_line(FZStream* stream, FlipperStreamWriteData* write_data) {
    bool result = false;
            ...
            for(size_t i = 0; i < data_size; i++) {
                ...
                if(result) {
                    ...
                    case FlipperStreamValueBool: {
                        ...
                        data[i] = !furi_string_cmpi_str(value, "true");
                        scan_values = 1;
                    }; break;
                    default:
                        // furi_crash("Unknown FF type");
...
bool flipper_format_stream_write_comment_cstr(FZStream* stream, const char* data) {
    bool result = false;
    do {
