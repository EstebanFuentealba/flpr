...
static void popup_view_draw_callback(Canvas* canvas, void* _model) {
    PopupModel* model = (PopupModel*)_model;
...
Popup* popup_alloc() {
    Popup* popup = (Popup*)malloc(sizeof(Popup));
    ...
    PopupModel * model = static_cast<PopupModel *>(view_get_model(popup->view));
    ...
    view_commit_model(popup->view, true);
    return popup;
}
...
void popup_reset(Popup* popup) {
    ...
    view_commit_model(popup->view, false);
    popup->callback = NULL;
    popup->context = NULL;
    popup->timer_enabled = false;
