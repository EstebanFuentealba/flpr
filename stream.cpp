...
static bool stream_seek_to_char_forward(FZStream* stream, char c) {
...
static bool stream_seek_to_char_backward(FZStream* stream, char c) {
    ...
    while(!result) {
        ...
        anchor -= to_read;
        stream_seek(stream, anchor, StreamOffsetFromStart);
        ...
        for(size_t i = 0; i < ret; i++) {
            ...
            if(buffer[cursor] == c) {
                result = true;
                stream_seek(stream, anchor + cursor, StreamOffsetFromStart);
...
size_t stream_read(FZStream* stream, uint8_t* data, size_t size) {
    // furi_assert(stream);
...
static bool stream_write_struct(FZStream* stream, const void* context) {
    // furi_assert(stream);
    // furi_assert(context);
...
size_t stream_write_format(FZStream* stream, const char* format, ...) {
    // furi_assert(stream);
    size_t size;
    va_list args;
...
size_t stream_load_from_file(FZStream* stream, Storage* storage, const char* path) {
    size_t was_written = 0;
    FZStream* file = file_stream_alloc(storage);

    do {
        if(!file_stream_open(file, path, FILE_WRITE)) break;
