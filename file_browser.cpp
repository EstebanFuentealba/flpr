...
static void BrowserItem_t_init_set(BrowserItem_t* obj, const BrowserItem_t* src) {
    ...
    if(src->custom_icon_data) {
        obj->custom_icon_data = (uint8_t *)malloc(CUSTOM_ICON_MAX_SIZE);
...
static void file_browser_scroll_timer_callback(void* context) {
    ...
    FileBrowserModel* model = (FileBrowserModel*)view_get_model(browser->view);
    model->scroll_counter++;
    view_commit_model(browser->view, true);
...
FileBrowser* file_browser_alloc(FuriString* result_path) {
    ...
    FileBrowser* browser = (FileBrowser*)malloc(sizeof(FileBrowser));
    ...
    if(!is_last) {
        ...
        if(is_folder) {
            ...
            if(browser->item_callback) {
                item.custom_icon_data = (uint8_t *)malloc(CUSTOM_ICON_MAX_SIZE);
                if(!browser->item_callback(
                       item_path,
        ...
        view_commit_model(browser->view, instant_update);
...
static void file_browser_view_draw_callback(Canvas* canvas, void* _model) {
    FileBrowserModel* model = (FileBrowserModel*)_model;
